		...
		if (token === mytoken) {
			...
			if (文件名 == 'config' || 文件名 == mytoken) {
				...
				if (request.method === 'POST') {
					try {
						// 尝试解析请求体
						const { text, b64 } = await request.json();

						// 如果 'b64' 为空，则以明文方式写入文件
						if (!b64) {
						...
						// 如果解析请求体失败
						return new Response('请求体格式错误，请确保是有效的 JSON', {
							status: 400,
							headers: { 'content-type': 'text/plain; charset=utf-8' },
						});
					}
...
function 下载bat(域名, token) {
	return [
		...
		`set "FILENAME=%~nx1"`,
		``,
		`rem 使用PowerShell安全地读取文件内容并生成JSON格式的请求体`,
		`powershell -Command "$content = (Get-Content -Path '%cd%/%FILENAME%' -Encoding UTF8) -join [Environment]::NewLine; $json = @{text = $content} | ConvertTo-Json -Compress; $url = 'https://%DOMAIN%/%FILENAME%?token=%TOKEN%'; $headers = @{'Content-Type'='application/json'}; Invoke-RestMethod -Uri $url -Method Post -Headers $headers -Body $json"`,
		``,
		`endlocal`,
	...
	return `
	  <html>
		...
		<body>
		  ...
		  token: ${token} <br>
		  <br>
		  <pre>注意! 上传时请使用 POST 请求，并在请求体中包含 JSON 数据。例如: {"text": "你的文本内容"} 或 {"b64": "你的 Base64 编码内容"}</pre><br>
